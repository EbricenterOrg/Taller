    ...
    public class csClienteBD
    {
      ...
      /*Metodo que obtiene los datos de la busqueda para que sean modificados */
        ...
        public static int Eliminar(int pId_cte)
        {
            ...
            MySqlCommand comando = new MySqlCommand(string.Format("Delete From tabt_det_cliente id_cte={0}", pId_cte), conexion);
            retorno = comando.ExecuteNonQuery();

            MySqlCommand comando2= new MySqlCommand(string.Format("DELETE FROM tab_cliente id_cte={0}",pId_cte), conexion);
            retorno = comando2.ExecuteNonQuery();
